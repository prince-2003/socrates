...
export default function Hero() {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    gallery.querySelectorAll(".floating-logo").forEach((element, index) => {
      const { parallaxSpeed } = itemPositions[index];
      const newX = (e.clientX - window.innerWidth / 2) * parallaxSpeed;
      const newY = (e.clientY - window.innerHeight / 2) * parallaxSpeed;
  ...
  useEffect(() => {
    ...
    if (smMediaQuery.matches) {
      galleryRef.current?.querySelectorAll(".floating-logo").forEach((element) => {
        gsap.to(element, {
          y: "+=20",
          repeat: -1,
          yoyo: true,
  ...
  return (
    <div className="min-h-screen w-[96vw] m-2 rounded-xl bg-black flex flex-col items-center relative justify-center overflow-hidden">
      <div className="relative flex flex-col items-center gap-10 ">
        ...
        <div className="heading opacity-0 flex gap-1 items-center bg-white/20 bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-xl px-4 py-2 text-white -mb-4 text-[.8rem] md:text-[1rem]  ">
          Discover the all new Socrates
          <div className="bg-white/20 bg-opacity-10 backdrop-filter backdrop-blur-lg  rounded-lg p-2 md:p-3  text-white">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="white"
              viewBox="0 0 448 512"
              className="w-[8px] md:w-[10px]"
            >
        ...
        <h1 className="heading text-4xl font-bold text-white text-center opacity-0 leading-tight z-10">
          One Coding Platform For Doing it <br /> All Together
        </h1>
        <span className="description w-[70%] text-white text-lg font-light text-center opacity-0 z-10">
      ...
      <div ref={buttonPrimaryRef} className="opacity-0 z-10">
  ...
  >
    ...
    <defs>
      ...
      <div ref={galleryRef} className="absolute z-10 inset-0 pointer-events-none">
  {items.map((item, index) => (
    <div
      key={item.src}
      className="floating-logo opacity-0 absolute bg-white/20 bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-xl border border-white pointer-events-none p-4 flex justify-center items-center" 
      style={{
